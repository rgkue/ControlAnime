...
const s = {
    ...
    loading:   false,
    tab:       'vistos',  // 'vistos' | 'pendientes' | 'likes'
...
function _datos() {
    if (!s.cache) return []
    const src = { vistos: s.cache.vistos, pendientes: s.cache.pendientes, likes: s.cache.likes }[s.tab] || []
...
function _actualizarTabs() {
    if (!s.cache) return
    const counts = { vistos: s.cache.vistos.length, pendientes: s.cache.pendientes.length, likes: s.cache.likes.length }
    ;['vistos','pendientes','likes'].forEach(key => {
    ...
    _el(`mlTab${s.tab.charAt(0).toUpperCase()+s.tab.slice(1)}`)?.classList.add('active')
}
